...
// Implementamos nuestro Constructor
    ...
    public function editar($idusuario, $nombre, $tipo_documento, $num_documento, $direccion, $telefono, $email, $cargo, $login, $clave, $imagen, $permisos) {
        ...
        // Eliminamos todos los permmisos asignados para volverlos a registrar
        $sqlDel = "DELETE FROM usuario_permiso WHERE idusuario='$idusuario'";
        ejecutarConsulta($sqlDel);
        //Asigno nuevamentelos permisos de los checkbox
        $num_elementos = 0;
        $sw = true;
        while ($num_elementos < count($permisos)) {
            $sql_detalle = "INSERT INTO usuario_permiso(idusuario, idpermiso) VALUES('$idusuario', '$permisos[$num_elementos]');";
    ...
    public function listarMarcados($idUsu) {
        $sql = "SELECT * FROM usuario_permiso WHERE idusuario='$idUsu';";
        return ejecutarConsulta($sql);
    }
